...
import { unscrambleSearch, copyText } from '../utils.js'
import UnscramblePopup from '../components/UnscramblePopup.jsx'
import styles from './GamePage.module.css'

function ResultRow({ anime, onExpand }) {
  ...
  return (
    <li className={styles.resultRow}>
      ...
      <button className={styles.expandBtn} onClick={() => onExpand(anime)}>
        EXPAND
      </button>
    </li>
  )
...
export default function UnscramblePage() {
  ...
  useEffect(() => {
    setResults(unscrambleSearch(query, animeData))
  }, [query])
  ...
  return (
    <div className={styles.page}>
      <div className={styles.inner}>
        <div className={styles.searchBox}>
          ...
          <input
            ...
            placeholder="Paste scrambled title e.g. HTTA ITME I TGO..."
            autoFocus
            spellCheck={false}
            autoComplete="off"
          />
        ...
        {results.length > 0 && (
          <div className={styles.resultsWrap}>
            ...
            <ul className={styles.results}>
              {results.map(a => (
                <ResultRow key={a.id} anime={a} onExpand={setExpanded} />
              ))}
            </ul>
          </div>
        )}
      ...
      {expanded && (
        <UnscramblePopup anime={expanded} onClose={() => setExpanded(null)} />
      )}
    </div>
  )
}
